% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% Modification to towards_olg function to decompose the welfare effects
% using CEV (with file B2CEV)
% Marta Oliva Riera
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
clear,clc

global nj ny replrate
...
elseif replrate == 0.6
    save vT.txt v -ascii -double
    save xT.txt x -ascii -double
end
...
global betta tetta r nj jr nx ny pi gridy netw pens sr epsi curv pini frac pop totpop grdfac L R replrate

close all

r = 0.0078;   % the one obtained from the GE
rho = 0.04;
betta = 1/(1+rho);
tetta = 2;
...
tau = 0.1746;   % also from GE with rr = 0
wage = 1.58;    % also from GE with rr = 0
netw=wage*(1-tau);
pens=replrate*wage*(1-tau);
epsi=ones(nj,1);
if (jr<nj),
    epsi(jr+1:nj)=0.0;
end;
