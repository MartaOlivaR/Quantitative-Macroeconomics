% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% Modification to towards_olg function into general equilibrium
% Marta Oliva Riera
% +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
clear,clc
...
% *************************************************************************
global maxit tol df r nj ny replrate
...
for it=1:maxit
    [fval, ass, Y, vfun, xgrid, wage, PhiAss] = func_olg(ret);
    display(abs(fval))
    if (abs(fval)< tol)             % check for convergence, if so = stop
        disp('convergence');    
        break;
    else                           % if not = update the initial guess of r 
        ret = ret - df * fval;      
...
% Input for consumption equivalent variation:
v = squeeze(vfun(1,:,:));
x = squeeze(xgrid(1,:,:));
if replrate == 0.0
    save v0.txt v -ascii -double
    save x0.txt x -ascii -double
elseif replrate == 0.6
    save v1.txt v -ascii -double
    save x1.txt x -ascii -double
end

% Gini coefficient: 
a = ass*PhiAss;         % distribution of assets
gini = giniindex(a);    

% ++++++++++++++++++++++++++++++++++++++++++++++++++++++

% ++++++++++++++++++++++++++++++++++++++++++++++++++++++
function [fval,ass,Y,vfun,gridx,wage,PhiAss] = func_olg(ret)

global delta L R replrate

mpk = ret + delta;          % marginal product of capital, based on the current guess for r (ret)

wage = func_firm(mpk);      % wage from the FOC of the firm

tau = func_pens(L,R,replrate);  % taxes on income

[gridx,gridsav,gridass,cfun,vfun] = func_hh(wage,ret,tau);    % solution to the household model, using w and guess on r

[Phi,PhiAss,ass] = func_aggr(gridx,gridsav,cfun,gridass,wage,ret,tau);

[mpk,Y] = func_mpk(ass, L);

retnew = mpk - delta;   

fval = ret - retnew;    % distance function
...
global betta alpha tetta delta df r nj jr nx ny pi gridy pens sr epsi curv pini frac pop totpop grdfac maxit tol L  R replrate
...
% ++++++++++++++++++++++++++++++++++++++++++++++++++++++
function [gridx,gridsav,gridass,cfun,vfun] = func_hh(wage,ret,tau)
% solution to the household problem:
global betta tetta nj nx ny pi gridy sr epsi curv grdfac replrate
...
for jc=nj-1:-1:1
    ...
    for yc=1:ny
        ...
        for xc=2:nx
            ...
            for ycc=1:ny
                % income tomorrow:
                incp1=epsi(jc+1)*wage*(1-tau)*gridy(ycc)+(1-epsi(jc+1))*replrate*wage*(1-tau);
